...
void ofApp::setup(){
    ...
    gui.add(radius.setup("radius",0.5,0.1,1));
    gui.add(r.setup("red", 150, 0, 255));
    gui.add(g.setup("green",150,0,255));
    gui.add(b.setup("blue",150,0,255));
...
void ofApp::draw(){
    ...
    for(int i =0; i< particles.size(); i++){
        ...
        particles[i].draw(radius,r,g,b);
        ofSetColor(r, g, b, 40);
        ofSetLineWidth(0.1);
        ofDrawLine(particles[i].pos, center);
    }
     gui.draw();
